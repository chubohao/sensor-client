...
class MPU9250:
    ...
    def searchDevice(self):
        ...
        if(who_am_i == DEVICE_ID):
            print("The mpusensor module initialization is complete.\n")
            return True
        else:
            print("Mpusensor not Connected.\n")
            return False
    ...
    def configMPU9250(self, gfs, afs):
        ...
        time.sleep(0.1)
        # Disable Gyro
        bus.write_byte_data(self.address, PWR_MGMT_2, 0x07)
        time.sleep(0.1)
        # gyro full scale select
...
if __name__=="__main__":
    ...
    while True:
        start = time.time()
        data_x, data_y, data_z = mpu9250.acc_sample()
        print(data_x[0], data_y[0], data_z[0])
        end = time.time()
        print("MPU: %.5f" % ((end - start)))